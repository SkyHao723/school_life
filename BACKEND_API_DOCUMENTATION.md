...
1. 用户通过 `/api/auth/send-code` 获取验证码
2. 用户通过 `/api/auth/register` 注册账号
3. 用户通过 `/api/auth/login` 登录获取JWT Token
4. 在需要认证的接口中，在请求头中添加 `Authorization: Bearer <token>`
...
**Endpoint**: `POST /api/user/verify-student`  
